...
const getAppUser = (req: Request) => (req as Request & { appUser?: AppUser }).appUser;
...
export const getOrganizationOverview = async (req: Request, res: Response) => {
  try {
    ...
    const orgResult = await pool.query(
      `SELECT id, "orgCode", name, "companyEmail", industry, "teamSize", "createdAt"
       FROM organizations
       WHERE id = $1 OR UPPER("orgCode") = UPPER($2)
       LIMIT 1`,
      [appUser.organizationId || null, appUser.organizationCode || null],
    );
    ...
    const departments = await pool.query(
      `SELECT d.id, d.name, d.description, d."createdAt", COUNT(u.id)::int AS "employeeCount"
       FROM departments d
       LEFT JOIN users u
    ...
    const employees = await pool.query(
      `SELECT id, name, email, role, department, provider, "createdAt"
       FROM users
    ...
    const hours = await pool.query(
      `SELECT COALESCE(SUM(t.hours), 0)::int AS "totalHours"
       FROM timesheets t
...
export const createDepartment = async (req: Request, res: Response) => {
  try {
    ...
    const inserted = await pool.query(
      `INSERT INTO departments ("organizationId", name, description)
       VALUES ($1, $2, $3)
       ON CONFLICT ("organizationId", name)
       DO UPDATE SET description = EXCLUDED.description
       RETURNING id, name, description, "createdAt"`,
      [appUser.organizationId, name.trim(), description?.trim() || null],
    );
